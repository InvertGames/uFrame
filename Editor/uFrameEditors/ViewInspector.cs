...
public class ViewInspector : uFrameInspector
{
  ...
  private void DoGroupField(KeyValuePair<string, List<FieldInfo>> groupField, ViewBase t)
  {
    if (EditorPrefs.GetBool(groupField.Key, false))
    {
      ...
      {
        FieldEditorGUILayout(field);
      }
    }
  }

  private void FieldEditorGUILayout(FieldInfo field)
  {
    try
    {
      ...
      // When implementing custom inspectors, this will find any method names in the inspector that match properties in the view model
      // otherwise use default uFrame EditorGUILayouts
      var inspectorType = GetType();
      var method = inspectorType.GetMethod(field.Name.Replace("_", string.Empty));
      if (method != null)
